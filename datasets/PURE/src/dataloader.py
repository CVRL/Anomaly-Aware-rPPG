...
class Dataset():
    ...
    def get_paths(self):
        data_paths = [os.path.join(self.input_root, f) for f in sorted(os.listdir(self.input_root))]
        out_paths = [f for f in sorted(os.listdir(self.input_root))]
        return data_paths, out_paths
    ...
    def __getitem__(self, idx):
        ...
        if self.return_video or self.return_waveform:
            ...
            video = data['video'] # (expected shape [T,H,W,C])
